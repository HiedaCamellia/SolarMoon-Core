...
public class EmbeddingTooltip extends BaseTooltip {
    ...
    public EmbeddingTooltip(BaseTooltipComponent tooltipComponent) {
        ...
        embeddingData = CapabilityUtil.getData(itemStack, SolarCapabilities.ITEMSTACK_DATA).getEmbeddingData();
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics guiGraphics) {
        if (canBeRendered()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                guiGraphics.blit(SolarBaseRes.EMBEDDING_SLOT, pX - 1, pY - 1, 0, 0, 18, 18);
                poseStack.translate(0, 0, 1000);
